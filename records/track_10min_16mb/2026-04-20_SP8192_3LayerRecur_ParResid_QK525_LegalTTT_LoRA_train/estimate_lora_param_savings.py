...
TRAIN_SCRIPT_PATH = SCRIPT_DIR / "train_gpt_decoded.py"


def load_train_module():
    spec = importlib.util.spec_from_file_location("train_gpt_decoded_local", TRAIN_SCRIPT_PATH)
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def parse_config_spec(spec: str) -> tuple[int, int, int]:
    parts = spec.split(":")
    if len(parts) == 2:
        model_dim, lora_rank = map(int, parts)
        embedding_dim = model_dim
        return model_dim, embedding_dim, lora_rank
    if len(parts) == 3:
        model_dim, embedding_dim, lora_rank = map(int, parts)
        return model_dim, embedding_dim, lora_rank
    raise ValueError(
        f"Invalid config spec {spec!r}. Use MODEL_DIM:LORA_RANK or MODEL_DIM:EMBEDDING_DIM:LORA_RANK."
    )


def numel_of(state_dict: dict[str, torch.Tensor], *, weights_only: bool = False) -> int:
    if weights_only:
        return sum(tensor.numel() for key, tensor in state_dict.items() if "weight" in key)
    ...
    *,
    ...
    if lora_rank > 0:
        ...
        adapters_only_state = ordered_subset(
            lora_raw_state,
            lambda key, _: key.endswith(".lora_A") or key.endswith(".lora_B"),
        )
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "train_gpt_decoded.py model with one or more MODEL_DIM / LORA_RANK configs."
        )
    )
    parser.add_argument(
        "--config",
        ...
        dest="configs",
        help=(
            "Configuration spec. Use MODEL_DIM:LORA_RANK or MODEL_DIM:EMBEDDING_DIM:LORA_RANK. "
...
def main() -> None:
    ...
    summaries = []
    for spec in configs:
        model_dim, embedding_dim, lora_rank = parse_config_spec(spec)
